...
export function HorizontalNavHeader() {
  return (
    <header className="w-full m-0 flex flex-row items-center bg-white rounded-md">
      <div className="container flex h-14 max-w-screen-2xl items-center">
        <div className="mr-4 hidden md:flex">
          ...
          <nav className="flex items-center gap-4 text-sm lg:gap-6">
            {routeInfo.map((route, routeIndex) => {
              return (
                <a
                  className="transition-colors hover:text-foreground/80 text-foreground/60"
                  href={route.href}
                  key={`navLink-${routeIndex}-${route.href}`}
                >
                  {route.linkText}
                </a>
              );
        ...
        <div className="flex flex-1 items-center justify-between space-x-2 md:justify-end">
          ...
          <nav className="flex items-center">
            <a
              target="_blank"
              rel="noreferrer"
              href="https://github.com/theednaffattack/note-taker"
            >
